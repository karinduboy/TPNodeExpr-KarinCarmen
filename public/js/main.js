...
const setModalContent = (modalFunction, userId) => {
	// estos contenidos podrian estar mejor divididos / componetizados
	let modalBtns = ''
	let modalContent = `
...
const checkPayload = (payload, userType) => {
	if (userType == 'New') {
		let isValid =
			!payload.name == '' && !payload.email == '' && !payload.address == '' && !payload.phone == '' ? true : false
		return isValid
...
const searchByQuery = () => {
	if (event.keyCode === 13) {
		...
		fetch(`/api/users/search/${query}`).then((res) => res.json()).then((data) => printUsers(data))
	}
}
...
const modalChange = (id) => {
	let userId = id
	fetch(`api/users/${id}`).then((res) => res.json()).then((res) => (userFound = res)).then((userFound) => {
